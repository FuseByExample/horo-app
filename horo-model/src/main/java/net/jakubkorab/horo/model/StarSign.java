...
public class StarSign {
	...
	public static StarSign addInstance(String signName, Integer startMonth, Integer startDay, Integer endMonth, Integer endDay) {
		StarSign sign = getInstance(signName);
		if (sign == null) {
			sign = new StarSign(signName, startMonth, startDay, endMonth, endDay);
	...
	public static StarSign getInstance(String signName) {
		...
		return signs.get(signName.toLowerCase());
	}
	...
	public static StarSign getInstance(ReadableDateTime dateTime) {
		...
		if (starSign == null) {
			throw new IllegalStateException("Unable to find star sign for " + DateTimeFormat.forPattern("yyyyMMdd").print(dateTime));
		}
		return starSign;
	}
	...
	static void resetSigns() {
		// there's no good reason to do this in a deployment - log if it does happen
	...
	public boolean appliesTo(ReadableDateTime dateTime) {
		...
		Interval signInterval = new Interval(
				start.toLocalDate(year).toDateMidnight(), 
